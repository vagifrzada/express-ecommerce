...
async function addToCart(user, product) {
    ...
    const foundItemIndex = cart.items
        .findIndex(item => item.product.equals(product._id));
...
async function deleteItemFromCart(user, product) {
    ...
    userCart.items = userCart.items.filter(item => !item.product.equals(product._id));
    user.cart = userCart;
    return await user.save();
}
...
async function resetPassword(user) {
    crypto.randomBytes(32, async (err, buffer) => {
        ...
        const passwordResetLink = process.env.APP_DOMAIN + `/auth/change-password/${user.passwordResetToken}`;
